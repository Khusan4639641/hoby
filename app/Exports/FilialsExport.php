...
class FilialsExport implements
    FromQuery,
    ShouldAutoSize,
    WithMapping,
    WithHeadings,
    ...
    public function query($data = [])
    {
        ...
        $query = OrderProduct::with(['order.contract','order.company','order.buyer','order.buyer.user','order'])
            ...
            ->leftJoin('contracts',function($query){
                $query->on('contracts.order_id', 'orders.id');
            })
            ->whereIn('orders.company_id',  $partners)
            ->select('order_products.*', 'orders.partner_id','contracts.canceled_at')
...
//            $product->order->contract->confirmed,
            ...
            $nds>0 ? NdsStopgagHelper::getActualNdsValue($product->order_created_at) . '%' : '0%', // M ндс %

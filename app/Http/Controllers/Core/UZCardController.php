...
class UZCardController extends CoreController {
    /**
     * @OA\Post(
     ...
     * @return mixed
     */
    public function sendSmsCodeUz(Request $request){


        Log::channel('cards')->info('start uzcard.cards.new.otp');
        ...
        $jRequest = json_encode([
            ...
            'params' => [
                'card' => [
                    'pan' => $split_info_card['card'],
                    'expiry' => $split_info_card['exp']
                ]
            ],
        ]);

        // отправка otp от UZCARD
    ...
    public function payment(Request $request){
        ...
        $token = EncryptHelper::decryptData($request->info_card['token']);
        $requestId = 'test' . uniqid(rand(),1) . $uid;
        $sum *= 100;
    ...
    public function scoring(Request $request){
        ...
        $sum = $request->sum;
        $requestId = 'test' . $request->buyer_id;

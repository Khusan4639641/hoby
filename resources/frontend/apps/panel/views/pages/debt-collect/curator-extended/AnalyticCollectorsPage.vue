    ...
    <a-table bordered :columns="columns" :loading="loading" :dataSource="debtors" :pagination="pagination" @change="onChange">
      <template #bodyCell="{text, column, record }">
        <template v-if="column.key === 'name'">
          <router-link :to="{ name: 'debt-collect-curator-extended-collector', params: { collectorId: record.id } }">
            {{ record.full_name }}
          </router-link>
        </template>
...
const loadDebtors = async () => {
  ...
  try {
    const { data } = await apiRequest.get('/v3/debt-collect-curator-extended/analytic/collectors', {params})

<template>
  <a-layout class="page contract-page">
    <a-page-header
      ...
      @back="() => router.push({ name: 'debt-collect-curator-extended-debtor', params: { debtorId: contract.debtor.id } })"
      >
    </a-page-header>
...
const loadContact = async () => {
  loading.value = true
  try {
    const { data } = await apiRequest.get(`/v3/debt-collect-curator-extended/contracts/${route.params.contractId}`)
    contract.value = data
    loading.value = false
  } catch (e) {
    console.error(e)

<template>
  <a-layout class="page debtor-page">
    ...
    <a-card :loading="loading">
      <template #actions>
        <calendar-outlined @click="isModalActive = true" />
      </template>
          ...
          <template #description>
            <a-card style="margin-top: 1rem;" :bordered="false">
              <a-card-grid
                  ...
                  @click="router.push({ name: 'debt-collect-curator-extended-contract', params: { contractId: contract.id } })"
              >
  ...
  <AddActionModal :active="isModalActive" @submit="handleAction" @cancel="isModalActive = false" />
</template>
...
import AddActionModal from '../../../../components/Contract/AddActionModal'
...
const loadDebtor = async () => {
  loading.value = true
  try {
    const { data } = await apiRequest.get(`/v3/debt-collect-curator-extended/debtors/${route.params.debtorId}`)
    debtor.value = data
    loading.value = false
  } catch (e) {
    console.log(e)
...
const loadDebtorContracts = async () => {
  try {
    const { data } = await apiRequest.get(`/v3/debt-collect-curator-extended/debtors/${route.params.debtorId}/contracts`)
...
const handleTransaction = async (type, data) => {
  try {
    await apiRequest.post(`/v3/debt-collect-curator-extended/debtors/${route.params.debtorId}/actions`, {
      type,
      content: data
    }, {
      headers: {
...
const handleAction = (data) => {
  handleTransaction('call_in_iiv', data)
      .then(() => isModalActive.value = false)
}

const goBack = () => {
  window.history.length > 0 ? router.back() : router.push({ name: 'debt-collect-curator-extended-debtors' })
}

<template>
    <a-layout class="page contract-page">
                            ...
                            <a-col :span="11">
                                ...
                                >
                                    ...
                                    <template #bodyCell="{ text, column, record }">
                                      ...
                                      <!-- <template v-if="column.key === 'region'">
                                        <div style="float:left;">{{ record.region?.name }}, {{ record.district?.name }} </div>
                                        <a-button type="dashed" style="float:right;"  @click="showRegionsModal(record, 'right')" size="small">
                                            Изменить
                                        </a-button>
                                      </template> -->
                                    </template>
      ...
      <!-- <SetRegionForExtendedModal url-prefix="debt-collect-curator-extended" :active="modals.regions" v-if="modals.regions && currentDebtor" :type="currentDebtorTableType" :debtor="currentDebtor" @submit="handleRegions" @cancel="hideRegionsModal" /> -->
    </a-layout>
  </template>
  ...
  <script setup>
    ...
    const currentDebtorTableType = ref(null)
    ...
    const loadCollectorsDebtors = async () => {
        ...
        try {
            const { data } = await apiRequest.get(`/v3/debt-collect-curator-extended/collectors/${route.params.collectorId}/debtors`, {params: {
    ...
    const loadAllDebtors = async () => {
        ...
        try {
            const { data } = await apiRequest.get(`/v3/debt-collect-curator-extended/collectors/${route.params.collectorId}/potential-debtors`, {params: {
    ...
    const attachDebtors = async (debtors) => {
      ...
      CDTData.loading = true
      try {
        const { data } = await apiRequest.post(`/v3/debt-collect-curator-extended/collectors/${route.params.collectorId}/debtors`, {debtor_id: debtors} )
    ...
    const detachDebtors = async (debtors) => {
      ...
      CDTData.loading = true
      try {
        const { data } = await apiRequest.delete(`/v3/debt-collect-curator-extended/collectors/${route.params.collectorId}/debtors`,{
          params: {
            debtor_id: debtors
          }
        })
    ...
    const loadCollectorData = async () => {
        loading.value = true
        try {
            const { data } = await apiRequest.get(`/v3/debt-collect-curator-extended/collectors/${route.params.collectorId}`)
    ...
    const goBack = () => {
      window.history.length > 0 ? router.back() : router.push({ name: 'debt-collect-curator-extended-collectors' })
    }
    ...
    // const showRegionsModal=(debtor, type)=>{
    //   currentDebtor.value = debtor
    //   currentDebtorTableType.value = type

    //   modals.regions = true
    // }
    // const hideRegionsModal=(debtor)=>{
    //   currentDebtor.value = null
    //   currentDebtorTableType.value = null
    // }
    // const handleRegions = async (type) => {

          ...
          <template #description>
            <a-card style="margin-top: 1rem;" :bordered="false">
              <a-card-grid
                  ...
                  @click="router.push({ name: 'debt-collect-curator-contract', params: { contractId: contract.id } })"
              >
...
const loadDebtor = async () => {
  loading.value = true
  try {
    const { data } = await apiRequest.get(`/v3/debt-collect-curator/debtors/${route.params.debtorId}`)
    debtor.value = data
    loading.value = false
  } catch (e) {
    console.log(e)
...
const loadDebtorContracts = async () => {
  try {
    const { data } = await apiRequest.get(`/v3/debt-collect-curator/debtors/${route.params.debtorId}/contracts`)
...
const handleTransaction = async (type, data) => {
  try {
    await apiRequest.post(`/v3/debt-collect-curator/debtors/${route.params.debtorId}/actions`, {
      type,
      content: data
    }, {
      headers: {
...
const goBack = () => {
  window.history.length > 0 ? router.back() : router.push({ name: 'debt-collect-curator-debtors' })
}

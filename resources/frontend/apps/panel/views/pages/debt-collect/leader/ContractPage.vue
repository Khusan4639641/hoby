...
const goBack = () => {
  window.history.length > 0 ? router.back() : router.replace({ name: 'debt-collect-leader-debtor', params: { debtorId: contract.debtor.id } })
}

const loadContact = async () => {
  loading.value = true
  try {
    const { data } = await apiRequest.get(`/v3/debt-collect-leader/contracts/${route.params.contractId}`)
    contract.value = data
    loading.value = false
  } catch (e) {
    console.log(e)

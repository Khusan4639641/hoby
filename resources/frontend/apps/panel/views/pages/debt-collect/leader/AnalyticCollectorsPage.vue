...
<!--            :disabled-date="monthsLimit"-->
<!--            picker="month"-->
<!--            @change="loadDebtors"-->
<!--        />-->
        <b>Скачать за:</b>
        <a :href="exportCollectorsLink(1)" target="_blank">
          <a-button type="primary">
            Прошлый месяц
          </a-button>
        </a>
        <a :href="exportCollectorsLink()" target="_blank">
          <a-button type="primary">
            Актуальный месяц
          </a-button>
        </a>
    ...
    <a-table bordered :columns="columns" :loading="loading" :dataSource="debtors" :pagination="pagination" @change="onChange">
      ...
      <template #footer>
        <b>Общая сумма:</b> {{ numberFormat.format(payments.total) }} сум
        <br>
        <b>Вознаграждение:</b> {{ numberFormat.format(payments.remunerations) }} сум
      </template>
    </a-table>
  </a-layout>
...
const loadDebtors = async () => {
  ...
  try {
    const { data } = await apiRequest.get('/v3/debt-collect-leader/analytic/collectors', {params})
...
const loadPayments = async () => {
  try {
    const { data } = await apiRequest.get('/v3/debt-collect-leader/analytic/collectors/payments')
    payments.total = data.total
    payments.remunerations = data.remunerations
  } catch (e) {
    console.log(e)
  }
}
...
const exportCollectorsLink = (offset = 0) => {
  const month = dayjs().subtract(offset, 'month').format('YYYY-MM');
  return `/api/v3/debt-collect-leader/analytic/export?month=${month}&api_token=${window.globalApiToken}`
}

<template>
  <a-layout class="page contract-page">
    <a-page-header
      ...
      @back="() => router.push({ name: 'debtor', params: { debtor: contract.debtor.id } })"
      >
    </a-page-header>
    <a-card :loading="loading">
      <a-card-grid v-if="contract?.debtor" :hoverable="false" style="width: 100%">
      ...
      <a-card-grid :hoverable="false" style="width: 100%">
        <a-card-meta title="История платежей">
          <template #description>
            <PaymentHistoryTable :payload="{ contract_id: route.params.contract}" />
          </template>
        </a-card-meta>
      </a-card-grid>
      <a-card-grid v-if="contract?.company" style="width: 100%">
        <a-card-meta title="Компания" v-if="contract?.company">
          <template #description>
            <p>
      ...
      <a-card-grid :hoverable="false" style="width: 100%; padding: 0">
        <a-collapse v-model="scheduleCollapse" ghost>
              <a-collapse-panel key="1" header="График платежей" class="ant-card-meta-title" style="padding: 12px 8px;">
                <ContractSchedulesTable :contractId="route.params.contract" />
              </a-collapse-panel>
        </a-collapse>
      </a-card-grid>
      ...
      <a-card-grid v-if="contract?.files" class="contract__document" v-for="file in files" :key="file.type">
...
const loadContact = async () => {
  loading.value = true
  try {
    const { data } = await apiRequest.get(`/v3/debt-collector/contracts/${route.params.contract}`)
    contract.value = data
    loading.value = false
  } catch (e) {
    console.error(e)
...
const handleTransaction = async (type, data) => {
  try {
    await apiRequest.post(`/v3/debt-collector/contracts/${route.params.contract}/actions`, {

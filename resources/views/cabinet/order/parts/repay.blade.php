    ...
    <div class="modal-dialog">
        <div class="modal-content" id="payment-method">
            <div class="modal-header">
                <h5 class="modal-title">{{__('cabinet/order.header_repay')}}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div v-if="!repaySuccess" class="modal-body">
                    ...
                    <div class="select-payment">
                        ...
                        <div class="form-check form-check-inline">
                            ...
                            <label class="form-check-label" for="paymentMethodAccount">
                                {{__('frontend/order.payment_account')}} ({{$order->buyer->settings->personal_account}} {{__('app.currency')}})
                            </label>
                        </div>
                    </div>

                    @if( count($order->buyer->cards) > 0 )

                    <div class="row" v-if="payment.isPaymentCard">
                        <div class="col-12 col-md-6">
                            <div class="title">{{__('cabinet/index.my_cards')}}</div>
                            <table class="table cards-list">
                                @foreach($order->buyer->cards as $index => $item)
                                    <tr>
                                        <td>
                    ...
                    @else
                        <div>{{ __('panel/buyer.txt_empty_card_list') }}</div>
                    @endif


                    @if( count($order->buyer->cards) > 0 || $order->buyer->settings->personal_account > 0 )
                    <div class="form-group">
                        <label>{{__('cabinet/order.txt_enter_summ')}}</label>
                        <number-input v-model="payment.total" :max-value="{{$order->contract->balance}}"/>
                    </div>
                    ...
                    @else
                        <div>{{ __('card.no_money') }}</div>
                    @endif

            </div>
            <div v-if="!repaySuccess" class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{__('app.btn_cancel')}}</button>
                <button type="button" class="btn btn-success" :disabled="smsCode == null || smsCode == ''" @click="checkSmsCode()">{{__('cabinet/order.btn_repay')}}</button>
            </div>
            <div v-if="repaySuccess" class="modal-body">
                <div v-for="item in messages" :class="'alert alert-' + item.type" v-html="item.text"></div>
            </div>
            <div v-if="repaySuccess" class="modal-footer">
                <a class="btn btn-primary" href="{{localeRoute('cabinet.orders.show', $order->id)}}">{{__('app.btn_reload_page')}}</a>
            </div>
        </div><!-- /.modal-content -->
    </div>
</div>
    ...
    var payment = new Vue({
        ...
        data: {
            ...
            payment: {
                ...
                isPaymentCard: true,
                typePayment: 'card',
                total: {{$order->contract->balance}},
                order_id: {{$order->id}},
                card_id: {{count($order->buyer->cards) >0 ? $order->buyer->cards[0]->id : 'null'}},
                personal_account: {{$order->buyer->settings->personal_account}},
            },
            resend: {
                interval: 60,
                indicator: false,
        ...
        methods: {
            ...
            pay: function () {
                let params = {
                    api_token: '{{$order->buyer->api_token}}',
                ...
                axios.post('/api/v1/order/payment/repay', params,

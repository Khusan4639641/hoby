            ...
            <div class="filtered" id="statistics">
                ...
                <div class="filter">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-4">
                            <div class="form-group">
                                <label for="report">{{__('billing/menu.statistics')}}</label>
                                <select id="report" name="report" v-model="filter.report" class="form-control select-report" @change="reportOnChange()">
                                    <option url="{{localeRoute( 'billing.reports.vendors.export', 'vendors' ) }}">{{__('statistics.partner.vendor_report')}}</option>
                                    <option url="{{localeRoute( 'billing.reports.vendorsCancel.export' ) }}">{{__('statistics.partner.report_on_vendors_with_cancelled')}}</option>

                                    <option url="{{
                                        localeRoute( 'billing.reports.delays.export', [
                                            "company_id"        => $company_id,
                                            "company_parent_id" => $company_parent_id,
                                            "user_id"           => $user_id
                                        ])
                                    }}">{{__('statistics.partner.report_on_delays')}}</option>
                                    @if($company)
                                    <option url="{{localeRoute( 'billing.reports.filials.export', 'filials' ) }}">{{__('statistics.partner.branch_report')}}</option>
                                    <option url="{{localeRoute( 'billing.reports.filialsCancel.export' ) }}">{{__('statistics.partner.report_on_branches_with_canceled')}}</option>
                                    @endif
                    ...
                    <div class="row">
                        <div class="col-12 col-md-8">
                            <button v-on:click="getReport" type="submit" class="btn btn-orange"> {{__('statistics.partner.btn_download')}}</button>
                        </div>
                    </div>
                </div>

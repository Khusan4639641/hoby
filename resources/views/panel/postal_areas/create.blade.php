...
@section('title', __('panel/postal_areas.header_create'))
@section('class', 'postal-areas edit')

@section('content')
    <div class="postal-areas">
        <form class="edit" method="POST" action="{{localeRoute('panel.postal-areas.store')}}">
            @csrf
                ...
                <div class="col form-group">
                    ...
                    <select class="form-control @error('katm_local_region') is-invalid @enderror" name="katm_local_region">
                        ...
                        @foreach($katm_local_regions as $key => $value)
                            <option @if($key == old('katm_local_region')) selected @endif value="{{$key}}">{{$value}}</option>
                        @endforeach
                    </select>
                ...
                <div class="col form-group">
                    ...
                    <select class="form-control @error('postal_region_id') is-invalid @enderror" name="postal_region_id">
                        ...
                        @foreach($postal_regions as $key => $value)
                            <option @if($key == old('postal_region_id')) selected @endif value="{{$key}}">{{$value}}</option>
                        @endforeach
                    </select>

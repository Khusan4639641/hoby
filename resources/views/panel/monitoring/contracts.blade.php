    ...
    <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
        <table class="table accounts-list dataTable no-footer" id="DataTables_Table_0" role="grid"
            ...
            <tr role="row">
                ...
                <th class="sorting_disabled" rowspan="1" colspan="1" style="width: 343px;">{{ __('Списано по контракту') }}</th>
                <th class="sorting_disabled" rowspan="1" colspan="1" style="width: 343px;">{{ __('Списано по плану') }}</th>
                <th class="sorting_disabled" rowspan="1" colspan="1" style="width: 343px;">{{ __('Списано по платежам') }}</th>
            </tr>
            </thead>
            <tbody>
            @foreach($contracts as $contract)
                <tr role="row" class="odd">
                    ...
                    <td class="text-right">{{ number_format($contract->contracts_sum, 2, ',', ' ') }}</td>
                    <td class="text-right">{{ number_format($contract->schedules_sum, 2, ',', ' ') }}</td>
                    <td class="text-right">{{ number_format($contract->payments_sum, 2, ',', ' ') }}</td>
                </tr>
            @endforeach

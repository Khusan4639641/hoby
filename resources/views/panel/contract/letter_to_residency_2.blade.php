...
@section('title', __('panel/contract.template_residency_letter_2'))
...
<style>
  ...
  <div id="letter_to_residency_2"  v-if="generalCompany">
    <div class="row">
      <div class="col-6">
          ...
          <div class="page-a4 residency" id="a4-doc" :class="{'border-0 pb-0 shadow-none': printing}">
            ...
            <section v-if="lang == 'ru'" class="letter">
              ...
              <p>
                ...
                соответствии с требованиями, предусмотренными в Оферте и договоре № <span>@{{ buyer.contract.id }}</span>
                от <span>@{{ moment(buyer.contract.created_at, 'DD.MM.YYYY').format('DD.MM.YYYY') }}</span>
                года, до сегодняшнего дня с вашей стороны не была произведена оплата поставленного товара.
              </p>
              <p>
                В результате несвоевременного выполнения своих обязательств, с вашей стороны имеется задолженность сроком
                <span>@{{ getDaysText(buyer?.contract?.expired_days_minus_one) }}</span>, которая
                на <span>@{{ moment(Date.now()).format('DD.MM.YYYY') }}</span> составляет
                <span>@{{ buyer?.contract?.total_max_autopay_post_cost || 0}}</span> сум, в связи с чем, согласно ст. 242
                Гражданского кодекса Республики Узбекистан, мы настоятельно просим произвести оплату в трёхдневный срок.
              </p>
              <p>
                В случае неисполнения Вами своих обязательств, мы будем вынуждены обратиться в  нотариальную контору
                или в гражданский суд для принудительного взыскания долга с Вас либо за счёт Вашего имущества.
              </p>
              <p>
                Вы можете оплатить задолженность путем пополнения своего ID
                (№<span>@{{buyer.phone}}</span> ) платежными системами CLICK, PAYME, APELSIN (resus BANK) или по следующим реквизитам в любом отделении Банка:
              </p>
            ...
            <section v-else class="letter">
              ...
              <p>
                Majburiyatlaringizni o'z vaqtida bajarmaganligingiz natijasida sizdan
                <span>@{{ moment(Date.now()).format('DD.MM.YYYY') }}</span> yil holatiga ko'ra
                <span>@{{ buyer?.contract?.expired_days_minus_one || 0 }} kun</span> kechiktirganingiz tufayli hozirda
                <span>@{{ buyer?.contract?.total_max_autopay_post_cost || 0 }}</span> so'm qarzdorlik yuzaga kelganligi sababli
                Sizdan 3 (uch) kun ichida to'lovni amalga oshirishni so'raymiz.</p>

              <p>Agar Siz o'z majburiyatlaringizni bajarmasangiz butun qarzni notarial idoraga yoki fuqarolik sudiga murojaat qilib undirish ishlarini amalga oshirishga majbur bo’lamiz, bu esa o'z navbatida qarzni sizdan yoki mol-mulkingiz hisobidan o'rnatilgan tartibda undiriladi.</p>
      ...
      <div class="modal fade" id="sendLetterModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
          <form @submit.prevent="sendLetter('letter-to-residency-2')" class="modal-content">
  ...
  <script>
    ...
    const letter_to_residency_2 = new Vue({
        el: "#letter_to_residency_2",

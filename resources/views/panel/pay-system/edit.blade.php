...
@section('title', $pay_system->title.' ('.__('panel/pay_sys.id').' '.$pay_system->id.')')
@section('class', 'partners edit')

@section('center-header-prefix')
    <a class="link-back" href="{{localeRoute('panel.pay_sys.show', $pay_system->id)}}"><img src="{{asset('images/icons/icon_arrow_green.svg')}}"></a>
@endsection

@section('content')

    <form method="POST" enctype="multipart/form-data" action="{{localeRoute('panel.pay-system.update', $pay_system)}}">
        @csrf
        @method('PATCH')
        ...
        <div class="form-controls">
            <a class="btn btn-outline-secondary" href="{{localeRoute('panel.partners.show', $pay_system->id)}}">{{__('app.btn_cancel')}}</a>
    ...
    <script>
        var app = new Vue({
            ...
            data: {
                ...
                files: {
                    new: null,
                    old: '{{$pay_system->logo->id??null}}',
                    delete: null
                },
                preview: '{{$pay_system->logo->preview??null}}',
            },
            methods: {
                resetFiles() {
                    this.preview = null;
